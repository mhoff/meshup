...
export default function CollectorClient() {
  ...
  const members = useMemo(
    () =>
      collectorState?.members.filter((m) => m.id !== clientMember?.id) || null,
    [collectorState, clientMember]
  );
  ...
  React.useEffect(() => {
    if (queryPeerId !== undefined) {
      const peerId =
        queryPeerId instanceof Array ? queryPeerId[0] : queryPeerId;
      runCollector(peerId);
      showNotification({
        id: "client-connect",
  ...
  return (
    <div>
      <div>
        ...
        <p>
          A host has provided you with this url to self-assess how well you are
          in touch with your colleagues. Start by choosing which team member you
          represent and then estimate how well you are connected to each other
          team member.
        </p>
      </div>
      ...
      {members && clientMember !== null && (
        <MemberRatings members={members} onRatingChange={setRatingCallback!!} />
      )}
    </div>
  );
}
...
CollectorClient.getLayout = function getLayout(page: React.ReactElement) {
  return <Shell nav={false}>{page}</Shell>;
};
