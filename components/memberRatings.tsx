...
function getClosestMark(val: number): string {
  const minMarkBy = R.minBy(({ value }: (typeof RATING_MARKS)[0]) =>
    Math.abs(val - value)
  );
  const mark = RATING_MARKS.reduce((a, b) => minMarkBy(a, b));
    ...
    setRatings(
      (prevRatings) =>
        new Map(
          (members || []).map((m) => [
            m.id,
            prevRatings.has(m.id) ? prevRatings.get(m.id) : undefined,
          ])
        )
    );
  }, [members]);
        ...
        {members &&
          members.map((m: Member) => (
            <tr key={m.id}>
              ...
              <td>
                <span>
                  {ratings.get(m.id) === undefined
                    ? "Unrated"
                    : getClosestMark(ratings.get(m.id)!!)}
                </span>
                <Slider
                  ...
                  onChange={(value: number) => {
                    setRatings(
                      (prevRatings) => new Map(prevRatings.set(m.id, value))
                    );
