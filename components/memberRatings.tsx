...
const RATING_MARKS = [
  ...
  { value: 100, label: 'Best buddies' },
];

function getClosestMark(val: number): string {
  const minMarkBy = R.minBy(({ value }: (typeof RATING_MARKS)[0]) => Math.abs(val - value));
        ...
        {members &&
          members.map((m: Member) => (
            <tr key={m.id}>
              <td>{m.name}</td>
              <td>
                <span>{ratings.get(m.id) === undefined ? 'Unrated' : getClosestMark(ratings.get(m.id)!!)}</span>
                <Slider
